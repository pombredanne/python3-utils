...
class _Heap:
    def __init__(self, heap_type, size=None):
        self._size = size
        self._heap = []
        self._reverse_heap = []
    ...
    def push(self, value, payload):
        # this is the value adjusted on whether this is a min or max heap
        adj_value = self._type * value

        if 0 < self.size == len(self._heap):
            # we are dealing with a limited size heap at capacity

            if (-1 * adj_value) <= self._reverse_heap[0]:
                # the value is too big (min heap) or small (max heap)
                # to fit in the limited size dict
                return False

            heap_elem = _HeapElement(adj_value, payload)
            reverse_heap_elem = _HeapElement(-1 * adj_value, payload)

            heapq.heapreplace(self._heap, heap_elem)
            heapq.heapreplace(self._reverse_heap, reverse_heap_elem)
        else:
            # the heap either has no size limit or is not full yet
            heap_elem = _HeapElement(adj_value, payload)
            reverse_heap_elem = _HeapElement(-1 * adj_value, payload)

            heapq.heappush(self._heap, heap_elem)
            heapq.heappush(self._reverse_heap, reverse_heap_elem)

        return True

    def __iter__(self):
        for e in sorted(self._heap, reverse=self.type < 0):
            yield (e.payload, e.value)
...
class MinHeap(_Heap):
    def __init__(self, size):
        super(MinHeap, self).__init__(heap_type=1, size=size)


class MaxHeap(_Heap):
    def __init__(self, size):
        super(MaxHeap, self).__init__(heap_type=1, size=size)
